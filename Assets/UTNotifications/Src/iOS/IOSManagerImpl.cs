...
namespace UTNotifications
{
    ...
    //public
        ...
        public override void PostLocalNotification(string title, string text, int id, IDictionary<string, string> userData, string notificationProfile)
        {
            if (m_enabled)
            {
                NotificationServices.PresentLocalNotificationNow(CreateLocalNotification(title, text, id, userData, notificationProfile));
            }
        }
        ...
        {
            LocalNotification notification = CreateLocalNotification(title, text, id, userData, notificationProfile);
        ...
        protected void LateUpdate()
        {
            ...
            if (localCount > 0)
            {
                foreach (var notification in NotificationServices.localNotifications)
                {
                    ...
                    ReceivedNotification receivedNotification = new ReceivedNotification(title, text, notification.userInfo.Contains(m_idKeyName) ? int.Parse((string)notification.userInfo[m_idKeyName]) : -1, userData, ExtractNotificationProfile(notification.soundName));

                    //Check if the notification was clicked
                    if (OnNotificationClickedHasSubscribers() && _UT_LocalNotificationWasClicked(receivedNotification.id))
                    {
            ...
            if (remoteCount > 0)
            {
                foreach (var notification in NotificationServices.remoteNotifications)
                {
                    ...
                    ReceivedNotification receivedNotification = new ReceivedNotification(title, text, m_nextPushNotificationId, userData, ExtractNotificationProfile(notification.soundName));
    ...
    //private
        private LocalNotification CreateLocalNotification(string title, string text, int id, IDictionary<string, string> userData, string notificationProfile)
        {
        ...
        {
            m_initRemoteNotifications = Settings.Instance.PushNotificationsEnabledIOS;
        ...
        private const string m_idKeyName = "_UT_ID";
        private const string m_providerName = "iOS";
