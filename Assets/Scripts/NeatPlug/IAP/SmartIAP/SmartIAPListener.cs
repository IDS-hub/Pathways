...
public class SmartIAPListener : MonoBehaviour {
	...
	public static SmartIAPListener INSTANCE = null;
	public static System.Action<bool, string> callback;
	...
	void OnItemDataReady() {
		...
		// Get item info:
		SmartIAPItem iapItem = SmartIAP.Instance().GetItemInfo("com.mymatenateapps.coin.1000");		
		...
		// Get item price:
		float itemPrice = SmartIAP.Instance().GetItemPrice("com.mymatenateapps.coin.1000");
	...
	void OnPurchaseCompleted(SmartIAPReceipt receipt) {
		...
		if (callback != null)
			callback(true, receipt.purchaseToken);
	}
	...
	void OnPurchaseFailed(string sku, string err) {
		...
		if (callback != null)
			callback(false, string.Format("Purchase failed, error -> {0}", err));			
	}
	...
	void OnPurchaseCancelled(string sku) {
		...
		if (callback != null)
			callback(false, string.Format("Purchase cancelled"));			
	}
	...
	void OnPurchaseCancelledByStore(string sku) {
		...
		if (callback != null)
			callback(false, string.Format("Purchase cancelled by store"));			
	}
	...
	void OnCompletedTransactionsRestored() {
		...
		if (callback != null)
			callback(true, string.Format("Transactions restored{0}", ""));
	}
	...
	void OnCompletedTransactionsFailedToRestore(string err) {
		...
		if (callback != null)
			callback(false, string.Format("Transactions failed to restore : {0}", err));	
	}
