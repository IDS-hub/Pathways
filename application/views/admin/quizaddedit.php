...
<div class="content">
	...
	//echo "2222"; die(); 
	//echo '<pre>'; print_r($tmpdetails1); die();
	//$countData = count($tmpdetails);
   foreach($tmpdetails1 as $tmpdetails){	
     //echo '<pre>'; print_r($tmpdetails); 
	?>
		...
		<form action="<?=$add_url;?>" method="post" accept-charset="utf-8" name="storefrm" id="storefrm2" enctype="multipart/form-data"  onsubmit="return validOne();">
				<input type="hidden" name="gift_id" value="<?=$tmpdetails['id']; ?>" />
            	<div class="form-group clearfix">
  	          	<div class="col-lg-2 noPaddAll">
  					<label for="exampleInputEmail1"><span class="redTxt">*</span>Question<?php //echo $tmpdetails['id'];  ?> :</label>
  	            </div>
  	            <div class="col-lg-4 noPaddAll">
  				<textarea name="question" cols="5" rows="5" id="question" class="form-control txtarea"><?php if(isset($tmpdetails['question'])){ echo $tmpdetails['question']; } ?></textarea>
				
				</div>
            	</div>
  			<div class="form-group clearfix">
  	          	<div class="col-lg-2 noPaddAll">
  					<label for="exampleInputEmail1"><span class="redTxt">*</span>CorrectAnswer<?php //echo $tmpdetails['id'];  ?>:</label>
  	            </div>
  	            <div class="col-lg-4 noPaddAll">
  					<input type="text" name="correctAnswer" id="correctAnswer" value="<?php if(isset($tmpdetails['correctAnswer'])){ echo $tmpdetails['correctAnswer']; } ?>"  class="form-control txtfield">
  				</div>
            	</div>
  			<div class="form-group clearfix">
  	          	<div class="col-lg-2 noPaddAll">
  					<label for="exampleInputEmail1"><span class="redTxt">*</span>Annotation<?php //echo $tmpdetails['id'];  ?>  :</label>
  	            </div>
  	            <div class="col-lg-4 noPaddAll">
  					
					<textarea name="annotation" cols="5" rows="5" id="annotation" class="form-control txtarea"><?php if(isset($tmpdetails['annotation'])){ echo $tmpdetails['annotation']; } ?></textarea>
  				</div>
            	</div>
