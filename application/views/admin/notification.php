     ...
     <div class="content">
	   ...
	   <div>
      	<ol class="breadcrumb">
			<li>Notification List<?php //echo $mainpagename1; ?></li>
		</ol>
		<?php $sessMsg = $this->session->userdata('msg'); ?>
		<?php if(isset($sessMsg)){ echo $sessMsg; }?>
	</div>
...
<div class="row clearfix">
	...
	<?php
		...
		$add_url=$this->config->item("base_url")."admin/notification/notifyaddedit";
		echo anchor($add_url, '<i class="fa fa-plus"></i>&nbsp; Add Notification', $stores_add_href_attribute);
		echo "&nbsp;";
	  ...
	  <li class="col-md-2" style="vertical-align:top;">
		<form role="form2" id="form2" action="" method="post">
		    ...
		    <?php
				...
				$type[100] = 100;
				$type[200] = 200;
				echo form_dropdown('type', $type, $this->session->userdata('sv_notifypageid'), "id='type' class='pageTyp form-control' onchange='myPagination(this.value);' style='background:#eeeeee;'");
			?>
			</div>
		</form>
		<li>
        ...
        <table class="table table-condensed table-striped border-top margin-none">
            <thead class="bg-gray borderRadius_3">
                <tr>
					...
					<!-- <th>Message</th> -->
					<!-- <th>Status</th> -->
					<th class="right-text" style="width: 40%;">Message</th>
                </tr>
            </thead>
            <tbody>
...
?>
                ...
                <tr>
					...
					<td><?php echo $i; ?></td>
					<td><?php echo date("Y-m-d H:i:s", strtotime($tmpdetails1['datetime'])); ?></td>
					<td><?php echo $tmpdetails1['title'] ; ?></td>
					<td><?php echo $tmpdetails1['message'] ; ?></td>
					<!-- <td><a href="<?php echo $this->config->item('base_url').'admin/notification/notifyaddedit/'.$tmpdetails1['id']; ?>">Edit</a> -->
                </tr>
                <!--Table row END -->
<?php
		$i++;
...
function myPagination(getval){
	$.ajax({
		type: "POST",
		url: "<?php echo $this->config->item('base_url').'admin/notification/setpagination';?>",
		cache: false,
		data: 'pageid='+getval,
		datatype: "text",
