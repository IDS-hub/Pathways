	...
	<div class="row">
	  <div class="col-md-12">
		<ul class="nav nav-tabs tabs" role="tablist">
		  <li role="presentation" class="tab"><a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">Follow</a></li>
		  <!-- <li role="presentation" class="tab active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">Featured</a></li> -->
		  <li role="presentation" class="tab active"><a href="#messages" aria-controls="messages" role="tab" data-toggle="tab">New</a></li>
		</ul>
		<div class="tab-content">
		  <?php /*?><div role="tabpanel" class="tab-pane active" id="home">
			<?php $featured = json_decode($featured); ?>
			<div class="card_container streamFeatured">
				<?php
				if(isset($featured->res) && $featured->res!='' && $featured->errorcode!=1){
					foreach($featured->res as  $val){ ?>
					  <div class="cardDiv" onclick="window.location='<?php echo $this->config->item('base_url')?>stream/index/<?=$val->id;?>'">
						<div class="card">
						  <div class="card-image">
							<div class="video-stream-img-holder"><img src="<?=(isset($val->user_details->cover_image) && ($val->user_details->cover_image!=''))?$val->user_details->cover_image:$this->config->item('base_url').'uploads/coverImage/no-image.png';?>"></div>
							<span class="btn-floating halfway-fab waves-effect waves-light white userView"><span class="userView_icon"><i class="material-icons dp48">people_outline</i></span><span class="userView_cont"><?=$val->view_cnt?></span></span>
						  </div>
						  <div class="card-content clearfix">
							<div class="userPic"><img class="responsive-img" src="<?=(isset($val->user_details->profile_image) && ($val->user_details->profile_image!=''))?$val->user_details->profile_image:$this->config->item('base_url').'uploads/user/no-image.png';?>"></div>
							<div><h2 class="userName truncate"><?php echo json_decode('"'.$val->user_details->first_name.' '.$val->user_details->last_name.'"');?> </h2>
							<p><i class="material-icons location_icon">location_on</i><?=isset($val->location)?$val->location:'';?></p></div>
						  </div>
						</div>
					  </div>
			  	<?php }
			...
			<?php if($featured->res!='' && count($featured->res)>9){?>
			<a class="feature-more" href="javascript:void(0);">More</a>
			<?php } ?>
		  </div><?php */?>
