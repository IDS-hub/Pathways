...
class Login_model extends CI_Model
{
	...
	public function getLastPainLevelUserfectchValue($userID){
		$this->db->select('*');
	...
	public function checkSessionPlayedStatus($userID,$session_id){
		$query = "SELECT isWatched FROM user_sessions WHERE user_id = '".$userID."' AND session_id='".$session_id."'";
		$rs = $this->db->query($query);
		if ($rs->num_rows() >0 ){
			//echo "1"; die();
			$row = $rs->row();
        ...
        public function checkUserReceiptTokenExist($userID) {
            $this->db->select('*');
            $this->db->from('subscription_purchase_details');
            $this->db->where('user_id', $userID);
        ...
        public function updateUserReceiptToken($userID,$receiptToken,$device_type,$subscriptionID) {
            $this->db->where('user_id', $userID);
            $data = array('receipt_token'=>$receiptToken,'device_type'=>$device_type,'subscription_id'=>$subscriptionID);
            $this->db->update('subscription_purchase_details', $data);

        }

        public function updateUserSubscriptionField($userID) {
            $this->db->where('id', $userID);
            $data = array('isSubscribed'=>'1');
            $this->db->update('user_tbl', $data);
			//echo $this->db->last_query(); die();

        }

        public function updateUserSubscriptionExpire($userID) {
            $this->db->where('id', $userID);
	...
	public function GetQuizDatafetch($session_id) {
		$this->db->select('*');
	...
	public function getLastSessionIDValuePainIncreaseDescr($userID) {
		$this->db->select('*');
		$this->db->from('user_sessions');
		$this->db->where('user_id', $userID);
		//$this->db->where('session_id>=', 63);
		//$this->db->where('session_id<=', 65);
		$this->db->where('session_id>=', 65);
		$this->db->where('session_id<=', 67);
		$this->db->order_by('session_id', 'DESC');
		$this->db->limit(1,0);
		$query = $this->db->get();
		//echo $this->db->last_query(); die();
	...
	public function deleteLastPainIncreaseDescSession($userID) {
		//echo $userID;
		//echo $LastSessionID; die();
	    $this->db->where('user_id', $userID);
		$this->db->where('session_id>=', 65);
		$this->db->where('session_id<=', 70);
	...
	public function getlastDecreasePainSessionGetValue($userID) {
		$this->db->select('*');
		$this->db->from('user_sessions');
		$this->db->where('user_id', $userID);
		//$this->db->where('session_id>=', 66);
		//$this->db->where('session_id<=', 68);
		$this->db->where('session_id>=', 68);
		$this->db->where('session_id<=', 70);
		$this->db->order_by('session_id', 'DESC');
		$this->db->limit(1,0);
	...
	public function getDataForSubscriptionEmail($userId){
		$arr = [];
		$this->db->select('*');
		...
		$query = $this->db->get();
		$res = $query->row();
		$arr['name'] = $res->first_name.' '.$res->last_name;
		$arr['email'] = $res->email;
		$this->db->select('*');
		$this->db->from('user_pains');
		...
		$query = $this->db->get();
		$res = $query->row();
		$arr['pain_date'] = $res->pain_date;
		$arr['pain_level'] = $res->pain_level;
		$arr['dignosis'] = $this->getDataForD($userId);
		return $arr;
	}
...
public function getReceiptTokenData($userID) {
	//echo $userID; die();
	$this->db->select('*');
	$this->db->from('subscription_purchase_details');
	$this->db->where('user_id', $userID);
	...
	public function updateSessionPainIncreDecrRequest($ID,$sessionID) {
		$this->db->where('id', $ID);
		$data = array('session_id'=>$sessionID,'isWatched'=>'0');
		$this->db->update('user_sessions', $data);
		return "0";
	}
	...
	public function getAdminInfo($val){
		$this->db->select('*');
	...
	public function getUserSessionDataUnplayed($userID) {
		$this->db->select('*');
		$this->db->from('user_sessions');
		$this->db->where('user_id', $userID);
		$this->db->where('isWatched', '0');
		...
		public function getUserSessionDataPlayed($userID) {
		$this->db->select('*');
		$this->db->from('user_sessions');
		$this->db->where('user_id', $userID);
		$this->db->where('isWatched', '1');
	    $query = $this->db->get();
		$res = $query->result_array();
		return $res;
	}
	...
	public function AveragePainGreaterconsiderationDate($userID,$LastConsiderationDateVal){
		$this->db->select('*');
		$this->db->from('userdaily_averagepain');
		$this->db->where('user_id', $userID);
		$this->db->where('created>=', $LastConsiderationDateVal);
		$this->db->where('created<', date('Y-m-d'));
		$query = $this->db->get();
		$res = $query->result_array();
	    return $res;
	...
	public function userRec($table_name,$id){
		...
		public function getUserDignosisAddedInfo($diagnosisIds) {
            $this->db->select('*');
            $this->db->from('diagnosis_tbl');
            $this->db->where('id', $diagnosisIds);
            $query = $this->db->get();
            $res = $query->result_array();
	    return $res;
        }
