...
//					}else{
				...
				if(count($usr_result)>0){
                    ...
                    if($valchk==1){
						...
						$msg .= '<p>We received a request to reset the password associated with this email address. If you made this request, please follow the instructions below.</p>
							...
							$msg .= '<p><a href="https://pathways.health/reset/index/'.$rnd.'" style="width:170px; height:32px; display:inline-block; text-align:center; border-radius:3px; line-height:32px; font-size:18px; font-family:Trebuchet MS, Arial, Helvetica, sans-serif; text-decoration:none; color:#FFF; background:#222;">Reset Password</a>
			                </p>
			                <p>Warm Regards <br> Pathways Team</p>';
	...
	public function PlaySession() {
        ...
        if($this->login_model->isValidAccessToken($accessToken)==0){
			...
			$playSessionStatus = $this->login_model->checkSessionExist($userID,$session_id);
			if($playSessionStatus>0){
...
public function getUserConsiderationPointVal($userID) {
	$LastConsiderationPointVal = $this->login_model->getLastConsiderationPointVal($userID);
	return $LastConsiderationPointVal;
}

public function getUserConsidirationRecord($LastConsiderationDateVal,$userID){
	//echo $LastConsiderationDateVal; echo '<br>';
	//echo $userID; die();
	if($LastConsiderationDateVal==Null){
		//echo "Null"; die();
		$numberofAveragePain = $this->login_model->numberofAveragePain($userID);
		//echo '<pre>'; print_r($numberofAveragePain);
		//echo count($numberofAveragePain); die();
		return $numberofAveragePain;
	}else{
		//echo "Not Null"; die();
		$AveragePain_considerationDate = $this->login_model->AveragePainGreaterconsiderationDate($userID,$LastConsiderationDateVal);
		//echo '<pre>'; print_r($AveragePain_considerationDate);
		//echo count($AveragePain_considerationDate); die();
		return $AveragePain_considerationDate;
	...
	public function AddPainIncreaseDecrease($userID, $session_id) {
		...
		if(count($numberofAveragePain)>1) {
			...
			$LastConsiderationPoint = $this->getUserConsiderationPointVal($userID);
			//print_r($LastConsiderationPoint); die();
			$LastConsiderationPointVal = $LastConsiderationPoint->last_consideration_value;
			$LastConsiderationDateVal = $LastConsiderationPoint->last_consideration_date;
			$considerationDateres = $this->getUserConsidirationRecord($LastConsiderationDateVal,$userID);
			//echo count($considerationDateres);
			$last_element = array_reverse( $considerationDateres )[0];
			//echo '<pre>'; print_r($last_element);
			//echo '<pre>'; print_r($considerationDateres); //die();
			if(count($considerationDateres)>=5){
				$FirstDate_pain_created = date('Y-m-d', strtotime($considerationDateres[0]['created']));
				$LastDate_pain_created = date('Y-m-d', strtotime($last_element['created']));
				...
				if($Datedifference_Pain_created>=5) {
				    //echo "OK"; die();
					$SIGNIFICANT_RISE_FALL = $last_element['average_pain']-$considerationDateres[0]['average_pain'];
					//echo $SIGNIFICANT_RISE_FALL; die();
					if($SIGNIFICANT_RISE_FALL>=+3 || $SIGNIFICANT_RISE_FALL<=-3){
					...
					$LastConsideration_Value = $last_element['average_pain'];
					$LastConsideration_Date = $last_element['created'];
	...
	public function SubscribeUser() {
        ...
        if($this->login_model->isValidAccessToken($accessToken)==0){
		    ...
		    if($subscribe_user){
				$tempData = $this->login_model->getDataForSubscriptionEmail($userID);
				$config2 = array();
				$config2['useragent'] = 'codeigniter';
				$config2['protocol']  = 'smtp';
				$config2['smtp_host'] = 'ssl://smtp.gmail.com';
				...
				$this->email->initialize($config2);
				$this->email->set_newline("\r\n");
				$this->email->from('randhir@isisdsn.com','Pathways');
				$this->email->to('randhir@isisdsn.com');

				$this->email->subject('New Customer');
				$msg = '<html><body>';
				$msg .= '<h3 style="color:#f40;">Hi Pathways!</h3>';
				$msg .= '<p>Name : '.$tempData['name'].'</p>
						<p>Email : '.$tempData['email'].'</p>
						<p>Dignosis : '.$tempData['dignosis'].'</p>
						<p>Pain Date : '.$tempData['pain_date'].'</p>
						<p>Pain Level : '.$tempData['pain_level'].'</p>
						<p>Plan : '.$subscribe_user.'</p>';
				$msg .= '<p></p>
					<p>Warm Regards <br> Pathways Team</p>';
				//$msg .= '<p style="color:#080;font-size:18px;">Your password is: '.$password.'</p>';
				$msg .= '</body></html>';
...
public function AddUserPain() {
	...
	public function SendUserFeedBack() {
        ...
        if($this->login_model->isValidAccessToken($accessToken)==0){
            ...
            $res['success'] = false;
			    ...
			    $sessionInfoFeedback = $this->login_model->getSpecificSessionInfo($session_id);
				//echo '<pre>'; print_r($sessionInfoFeedback); die();
				$sessionName = $sessionInfoFeedback[0]['title'];
				$lastPainLevelFetch = $this->login_model->getLastPainLevelUserfectchValue($userID);
				//echo '<pre>'; print_r($lastPainLevelFetch);  die();
				$lastPainLevelFetchValue = $lastPainLevelFetch[0]['pain_level'];
				...
				foreach($userDignosisAll as $userDignosis){
					$diagnosisIds = $userDignosis['diagnosis_id'];
					$UserDiganosisTitleFetch.= $userDignosis['diagnosis_title'].',';
					if($diagnosisIds!=0){
						//echo "sdfdsafsdf"; die();
				...
				if(count($usr_result)>0){
					...
					$userEmail = $usr_result->email;
					$painlevel =$usr_result->last_consideration_value;
					$SubType =$usr_result->isSubscribed;
					if($SubType=='0'){
						$SubTypeValue = "Not Subscribed";
					}else{
						$SubTypeValue = "Subscribed";
					}
                    if($userEmail){
						$config2 = array();
						$config2['useragent'] = 'codeigniter';
						...
						$msg .= '<p>User name: '.$userName.'</p>
								...
								<p>Subscription Type: '.$SubTypeValue.'</p>
								<p>Condition: '.rtrim($UserDiganosisTitleFetch,",").'</p>
								<p>Last pain level: '.$lastPainLevelFetchValue.'</p>
	...
	public function GetQuizData() {
			$session_id  = $this->input->post('session_id');
			//echo $session_id; die();
			$getQuizData = $this->login_model->GetQuizDatafetch($session_id);
			//echo '<pre>'; print_r($getQuizData); die();
			$getQuizDatacount = count($getQuizData);
			$res = array();
			if($getQuizDatacount>0){
				$qiz = 0;
				foreach($getQuizData as $quiz){
					$qanswer = json_decode($quiz['correctAnswer']);
					if($qanswer==1){
						$CorrectAnswer = true;
					}else{
						$CorrectAnswer = false;
					}
					$res[$qiz]['Question'] = $quiz['question'];
					$res[$qiz]['CorrectAnswer'] = $CorrectAnswer;
					$res[$qiz]['Annotation'] = $quiz['annotation'];
					$qiz++;
				}
			}else{
				$res['success'] = false;
