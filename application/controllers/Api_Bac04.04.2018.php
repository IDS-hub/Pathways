...
//					}else{
						    ...
						    if($val['session_id']==5) {
								...
								if(is_array($sessionData11)) {
								  /*foreach($sessionData11 as $fifthdata){
									...
									$sessionData2['isWatched'] = $val['isWatched'];
									//$sessionData2['session_description'] = mysql_real_escape_string($val['session_description']);
									...
									$sessionData2->isWatched = $val['isWatched'];
									//$sessionData2->session_description = preg_replace("/[^a-zA-Z]+/", "", $val['session_description']);
									$sessionData2->updated = $val['updated'];
								}
								...
								//$sessionData[0]['session_description'] = preg_replace("/[^a-zA-Z]+/", "", $val['session_description']);
	...
	public function PlaySession() {
        ...
        if($this->login_model->isValidAccessToken($accessToken)==0){
            ...
            $res['success'] = false;
			    ...
			    $this->AddPainIncreaseDecrease($userID, $session_id);
	...
	public function AddPainIncreaseDecrease($userID, $session_id) {
		...
		if(count($numberofAveragePain)>0) {
			...
			if($LastConsiderationPointVal->last_consideration_value==0||$LastConsiderationPointVal->last_consideration_value==NULL) {
				//echo "NULL"; die();
