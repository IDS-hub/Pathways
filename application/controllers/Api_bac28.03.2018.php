			...
			if(count($check_session) >0) {
                    foreach($check_session as $val){
						    if($val['session_id']==5) {
								...
								$sessionData11 = $this->login_model->getFifthSessionInfo($fifthSessionID,$Userdiagnosis_ID);
								//echo '<pre>'; print_r($sessionData11); 
								if(count($sessionData11) > 0) {
									//echo "OKKK";
								$sessionData2 = $sessionData11;
								//$sessionData2['id'] = $sessionData11->session_id;
								
								//$sessionData2['isWatched'] = $val['isWatched'];
								//$sessionData2['audio_url'] = $this->config->item('base_url').'uploads/sound_files/'.$sessionData2[0]['audio_url'];
								...
								$sessionData[0]['audio_url'] = $this->config->item('base_url').'uploads/sound_files/'.$sessionData[0]['audio_url'];
        ...
        if($this->login_model->isValidAccessToken($accessToken)==0){
            ...
            $res['success'] = false;
				...
				if(count($user_pain_hit) > 0) {
					...
					//print_r($data); die();
					$this->login_model->user_pain_num_hit_update($userID, $data);
					$res['success'] = true;
				} else {
					//echo "2"; die();
					$data['user_id'] = $userID;
            ...
            $res['success'] = false;
				...
				if(count($user_pain_Info) >0) {
                    ...
                    foreach($user_pain_Info as $val){
                            ...
                            $res2[$p]['sessionDate'] = $val['created'];
